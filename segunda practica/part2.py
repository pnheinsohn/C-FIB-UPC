...
aes = AES(mode='ecb', input_type='hex')
intToHex = lambda V, n: "{0:0{1}x}".format(V,n)
# Test vector 128-bit key
key = '220464924484130500fe2a3b9997abcd'
# Encrypt data with your key
Muestra = '15337eb3971c6deac4c21b3bef8b2e95'


""" En este apartado si ponemos la funcion SubBytes igual a la identidad, vemos que checkApartado1
devuelve true y por lo tanto se cumple la condicion que queriamos comprobar. Por otro lado si
dejamos la funcion SubBytes original nos devuelve False. Para hacer la prueba solo hace falta
descomentar la linia return state de la funcion SubBytes"""

def checkApartado1(Muestra):
    #Generamos el resultado de encriptar la muestra
    C = int(aes.encryption(Muestra, key), 16)
    M = int(Muestra,16)
    #Recorremos los 128 bits de la muestra para cambiarlos uno a uno
    for i in range(128):
        #Modificamos la muestra haciendo un XOR y asi modificando el bit i
        Mi = intToHex(M ^ (1 << (127 - i)),32)
        Ci = int(aes.encryption(Mi, key), 16)
        for j in range(i+1,128):
            Mj = intToHex(M ^ (1 << (127 - j)),32)
            Mij = intToHex(M ^ (((1 << (127 - i)) | (1 << (127 - j)))),32)
            Cj = int(aes.encryption(Mj, key), 16)
            Cij = int(aes.encryption(Mij, key), 16)

            if (C != Ci ^ Cj ^ Cij): return False
    return True

print("Resultado del Apartado 1 es " + str(checkApartado1(Muestra)))



""" En este segundo y tercer apartado generamos 128 muestras aleatorias, y hacemos XOR entre los
resultados de encriptar la muestra original y la muestra con un bit cambiado
    En el caso que analizamos ShiftRows vemos que con la funcion original el XOR nos da resultados
    como:
        5636bc48d6dc09477dd4a816b88c534b
    Es decir que los 2 mensajes encriptados son bastante distintos. En cambio si ShiftRows
    no hace nada el resultado de la XOR es por ejemplo:
        00000000afa76a410000000000000000
    Y como vemos eso significa que C y Ci son mucho mas parecidos que antes

    Finalmente si en lugar de ShiftRows convertimos MixColumns en identidad nos da un resultado parecido:
        0000000000000000002e000000000000
    pero se puede ver que C y Ci son aun mas parecidos que en el caso anterior."""

def checkApartado2y3(Muestra):
    #Generamos 128 muestras aleatorias
    for i in range(128):
        M = int(Muestra,16)-random.randint(1,10000)
        C = aes.encryption(intToHex(M,32), key)
        Mi = intToHex(M ^ (1 << (127 - i)),32)
        Ci = aes.encryption(Mi, key)
        r = intToHex((int(C,16) ^ int(Ci,16)), 32)
        #print(str(r))

checkApartado2y3(Muestra)


##############  PARTE 2.2  ##############

#Funcion para calcular el numero de unos
def Num1(X):
    res = 0
    while X != 0:
        res += X%2
...
#Funcion devuelve lista posiciones que han cambiado
def NumP(X):
    res = []
    i = 0
    while X != 0:
...
""" En la funcion cambio_bits_mensaje() hemos experimentado que efectos tiene en el mensaje encriptado
variar un bit del mensaje original. Hemos cambiado todos los bits del mensaje, realizando asi 128 pruebas
y hemos generado 2 graficos: el primero nos permite ver el numero de bits que han cambiado en cada caso
y el segundo la frecuencia con la que varia cada posicion. Estos graficos se pueden ver en el fichero GraficosParte2.pdf
En la cambio_bits_key() haremos lo mismo pero cambiando un bit de la key en el cual se vera que el numero de cambios de bit
augmenta """

def cambio_bits_mensaje():
    histogramaBits = []
    histogramaPos = {}

    Mstr = intToHex(int(Muestra,16),32)
    C = int(aes.encryption(Mstr, key), 16)

    for i in range(128):
        Mi = intToHex(int(Muestra,16) ^ (1 << (127 - i)), 32)
        Ci = int(aes.encryption(Mi, key), 16)

        numBits = Num1(Ci ^ C)
        numPos = NumP(Ci ^ C)

        histogramaBits.append(numBits)
        for j in numPos:
            if j in histogramaPos:
                histogramaPos[j] += 1
            else:
                histogramaPos[j] = 1

    #Cada columna marcara el numero de bits que ha cambiado en las 128 modificaciones
    plt.hist(histogramaBits, alpha=1, edgecolor = 'black',  linewidth=1)
    plt.xlim(xmin=0, xmax=128)
    plt.grid(True)
    plt.show()

    plt.bar(histogramaPos.keys(), histogramaPos.values())
...
def cambio_bits_key():
    ...
    for i in range(128):
        k = intToHex(ky ^ (1 << (127 - i)), 32)
        Ci = int(aes.encryption(Mstr, k), 16)
...
"""En esta tercera parte como queremos buscar un mensaje cifrado con el maximo numero de ceros, la forma
mas rapida de encontrar un M y k para generarlo es descifrar un C con todo 0 y una llave k cualquiera y asi sacar
el M que si lo cifraramos con k nos daria el numero maximo de 0 en C. Por lo tanto solo hacemos una prueba y el maximo
numero de 0 es 128"""

def parte2_3():
    k = '220464924484130500fe2a3b9997abcd'
    C = '00000000000000000000000000000000'
    M = int(aes.decryption(C,k),16)
    #print(intToHex(M,32))
    #C2 = int(aes.encryption(intToHex(M,32),k),16)
    #print(intToHex(C2,32))
